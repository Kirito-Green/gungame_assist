...
def on_press(key):
    ...
    if hasattr(key, 'char') and key.char == key_eaglevision: # 侧键锁头并开枪
        # lock head
        global lock_head_flag
        if not lock_head_flag:
            lock_head_flag = 1
        img = get_screenshot(width, height)
        person_pos = get_head_pos(img)
        if len(person_pos) > 0:  # 1 17 3
            for head_pos in person_pos[0][0:5]: # head
                if head_pos[2] >= score_thres:  # y x
                    head_x = int(x0 + head_pos[1] * width)
                    head_y = int(y0 + head_pos[0] * height)
                    mouse.position = (screen_center_width, screen_center_height)
                    driver.move_R(int(head_x - screen_center_width), int(head_y - screen_center_height))
                    # linear_interpolation(int(head_x-screen_center_width), int(head_y-screen_center_height), num_steps=7, delay=0.01)
                    # mouse_move_PID(mouse, head_x, head_y)
...
def on_release(key):
    if hasattr(key, 'char') and key.char == key_eaglevision: # 结束锁头
        global lock_head_flag
        lock_head_flag = 0
